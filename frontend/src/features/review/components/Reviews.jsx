...
export const Reviews = ({ productId, averageRating }) => {
  ...
  const reviews = useSelector(selectReviews) || []; // Fallback to empty array
  ...
  // Filter valid reviews for rating counts - ensure both review AND review.user are valid
  const validReviews = reviews.filter((review) => review && review._id && review.rating && review.user && review.user._id);
          ...
          <Stack rowGap={3}>
            <Stack rowGap={1}>
              ...
              <Typography variant="h6" color={'text.secondary'}>
                Based on {validReviews?.length} {validReviews?.length === 1 ? 'Review' : 'Reviews'}
              </Typography>
            </Stack>
            <Stack rowGap={2}>
              {[5, 4, 3, 2, 1].map((number) => (
                ...
                >
                  ...
                  <LinearProgress
                    ...
                    variant="determinate"
                    value={(ratingCounts[number] / validReviews.length) * 100}
                  />
                  <Typography>{parseInt((ratingCounts[number] / validReviews.length) * 100)}%</Typography>
                </Stack>
              ))}
            </Stack>
          </Stack>
      ...
      {writeReview ? (
        <Stack rowGap={3} position={'relative'} component={'form'} noValidate onSubmit={handleSubmit(handleAddReview)}>
          ...
          <Stack>
            ...
            <motion.div style={{ width: 'fit-content' }} whileHover={{ scale: 1.050, x: 2 }} whileTap={{ scale: 1 }}>
              <Rating size="large" value={value} onChange={(e) => setValue(Number(e.target.value))} />
            </motion.div>
          </Stack>
        ...
        <motion.div onClick={() => setWriteReview(!writeReview)} whileHover={{ scale: 1.050 }} whileTap={{ scale: 1 }} style={{ width: 'fit-content' }}>
          <Button
            ...
            variant="contained"
            sx={{ color: theme.palette.primary.light, textTransform: 'none', fontSize: '1rem', borderRadius: '6px' }}
